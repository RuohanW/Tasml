...
class DataProvider(object):
  ...
  def __init__(self, dataset_split, config, verbose=False, load_path=None):
    ...
    if load_path:
      with open(load_path, "rb") as f:
        self.fixed_dataset = pickle.load(f)
  ...
  def create_fixed_dataset(self, size, num_classes, tr_size, val_size, full=True, save_path=None):
    def _build_one_instance_py():
      ...
      # if normalize:
      #   embedding_array = np.array([[embeddings[image_path] / np.linalg.norm(embeddings[image_path])
      ...
      embedding_array = np.array([[embeddings[image_path]/ np.linalg.norm(embeddings[image_path])
                                  ...
                                  for class_paths in path_array]).astype(np.float64)
      if full:
        task_signature = np.mean(embedding_array, axis=(0, 1))
      else:
        task_signature = np.mean(embedding_array[:, :tr_size], axis=(0, 1))
      if self._verbose:
        tf.logging.info(task_signature.shape)
      return task_signature, label_array, path_array
